...
class AppDelegate: UIResponder, UIApplicationDelegate, TVApplicationControllerDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        appControllerContext.javaScriptApplicationURL = Bundle.main.url(forResource: "application", withExtension: "js")!
        let rootURL = appControllerContext.javaScriptApplicationURL.deletingLastPathComponent()
        appControllerContext.launchOptions["BASEURL"] = rootURL.absoluteString
    ...
    func loadHome() {
        ...
        self.appController?.evaluate(inJavaScriptContext: { (context) in
            
            let carousel = false // true = carousel, false = grid
            let showHomeTemplate = context.objectForKeyedSubscript("showHomeTemplate")
            _ = showHomeTemplate?.call(withArguments: [content, carousel]) //
            
        }, completion: { (success) in
            print("ShowHomeTemplate success: \(success)")
        })
    }
}
